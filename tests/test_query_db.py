...
class TestQueryDB:
    ...
    def data_collector(self):
        """创建数据收集器实例"""
        return create_data_collector(db_type=DB_TYPE,
                                     buffer_size=1,
                                     db_path=DB_PATH)
    ...
    @pytest.mark.skipif(not os.path.exists('../db'),
                        reason="No database directory found")
    def test_main_no_table_interactive_quit(self):
        ...
        result = subprocess.run([sys.executable, query_db_path],
                                capture_output=True,
                                text=True)
        ...
        if os.path.exists(db_path):
            ...
            if tables:
                ...
                result = subprocess.run([
                    sys.executable, query_db_path, '--table', table_name,
                    '--limit', '3'
                ],
                                        capture_output=True,
                                        text=True)
