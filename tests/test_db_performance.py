...
class TestDBPerformance:
    ...
    def generate_test_data(self, num_records: int) -> list:
        ...
        for i in range(num_records):
            ...
            # 生成随机的盘口数据
            bid_prices = [round(last_price - random.uniform(0, 10), 2)
                          for _ in range(5)]
            ask_prices = [round(last_price + random.uniform(0, 10), 2)
                          for _ in range(5)]
...
def main():
    ...
    parser.add_argument('--records', '-r', type=int,
                        default=10000, help='测试数据记录数，默认10000条')
    parser.add_argument('--buffer', '-b', type=int,
                        default=128, help='缓冲区大小，默认128')
    parser.add_argument('--read_limit', '-l', type=int,
                        default=1000, help='读取测试的记录数，默认1000条')
