...
class TestDataCollector(unittest.TestCase):
    ...
    def test_data_collector_single_process(self):
        ...
        with contextlib.ExitStack() as stack:
            ...
            mock_run = stack.enter_context(mock.patch.object(self.app._trading_client, 'run'))
            # 模拟logger.set_log_file和set_log_level方法
            stack.enter_context(mock.patch.object(main_logger, 'set_log_file'))
            stack.enter_context(mock.patch.object(main_logger, 'set_log_level'))
            
            # 调用data_collector方法（单进程模式）
            self.app.data_collector(platform="TEST", env="test", collector_id="test_id", count=1)
            
            # 验证trading_client.run是否被正确调用
            mock_run.assert_called_once_with("TEST", "test", api_type="md")
    ...
    def test_data_collector_multi_process_logging(self):
        ...
        with contextlib.ExitStack() as stack:
            # 模拟UUID生成
            stack.enter_context(mock.patch('uuid.uuid4', side_effect=mock_uuids))
            # 模拟multiprocessing.Process
            mock_process = stack.enter_context(mock.patch('multiprocessing.Process'))
            ...
            # 验证每个进程都传递了正确的target和args
            for i in range(3):
                call_args = process_calls[i][1]
                self.assertEqual(call_args['target'], self.app._process_manager._data_collector_process)
                self.assertEqual(call_args['args'], ('TEST', 'test', mock_uuids[i]))
    ...
    def test_data_collector_process_log_file(self):
        ...
        with contextlib.ExitStack() as stack:
            # 模拟UUID生成
            stack.enter_context(mock.patch('uuid.uuid4', return_value=mock_uuid))
            # 模拟config模块中的LOG_PATH
            stack.enter_context(mock.patch('utils.process.LOG_PATH', '/mock/logs'))
            # 模拟os.path.join函数
            mock_path_join = stack.enter_context(mock.patch('os.path.join', side_effect=lambda *args: '/'.join(args)))
            # 模拟trading_client.run方法
            stack.enter_context(mock.patch.object(self.app._trading_client, 'run'))
            # 模拟logger.set_log_file和set_log_level方法
            mock_set_log_file = stack.enter_context(mock.patch.object(main_logger, 'set_log_file'))
            stack.enter_context(mock.patch.object(main_logger, 'set_log_level'))
            ...
            # 验证日志文件路径被正确构造
            mock_path_join.assert_any_call('/mock/logs', f'data_collector_{mock_uuid}.log')
            
            # 验证设置了正确的日志文件
            mock_set_log_file.assert_any_call('/mock/logs/data_collector_test-uuid-123.log')
    ...
    def test_data_collector_parameters(self):
        ...
        with contextlib.ExitStack() as stack:
            ...
            self.app._trading_client.run.assert_called_once_with(
                "CUSTOM_PLATFORM", "custom_env", api_type="md"
            )
    ...
    def test_logger_daily_rotation(self):
        ...
        with mock.patch('utils.logger.datetime') as mock_datetime:
            ...
            expected_path_1 = os.path.normpath("/mock/logs/data_collector_test-uuid_2025-12-13.log")
            ...
            # 验证日志文件路径（使用os.path.normpath处理路径分隔符差异）
            expected_path_2 = os.path.normpath("/mock/logs/data_collector_test-uuid_2025-12-14.log")
            self.assertEqual(os.path.normpath(second_log_file), expected_path_2)
