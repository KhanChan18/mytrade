...
def test_data_collector_method():
    ...
    assert hasattr(client, 'data_collector'), "MyTradeApp should have data_collector method"
    
    # 验证data_collector是可调用的
    assert callable(getattr(client, 'data_collector')), "data_collector should be callable"
...
def test_trade_controller_method():
    ...
    assert hasattr(client, 'trade_controller'), "MyTradeApp should have trade_controller method"
    
    # 验证trade_controller是可调用的
    assert callable(getattr(client, 'trade_controller')), "trade_controller should be callable"
