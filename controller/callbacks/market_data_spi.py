...
class MarketDataSpi(mdapi.CThostFtdcMdSpi):
    ...
    def OnFrontConnected(self):
        """行情连接成功"""
        main_logger.info("MDController", "Front server connected, starting login")
    ...
    def OnRspUserLogin(self, pRspUserLogin, pRspInfo, nRequestID, bIsLast):
        ...
        main_logger.info("MDController", f"Login successful, trading day: {trading_day}")
    ...
    def OnRtnDepthMarketData(self, pDepthMarketData):
        ...
        if pDepthMarketData is None:
            main_logger.error("MDController", "Market data push: pDepthMarketData is None")
            return
    ...
    def OnRspSubMarketData(self, pSpecificInstrument, pRspInfo, nRequestID, bIsLast):
        ...
        # 只在最后一个响应时记录订阅总数
        if bIsLast:
            main_logger.info("MDController", f"Successfully subscribed to {self.controller.subscribed_count} out of {self.controller.total_to_subscribe} market data contracts")
