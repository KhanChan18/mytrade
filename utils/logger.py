...
class Logger:
    ...
    def __init__(self, log_file=None, log_level="INFO"):
        ...
        """
        self._base_log_file = log_file  # 基础日志文件路径
        self._current_log_file = log_file  # 当前使用的日志文件路径
        self._current_log_date = None  # 当前日志文件对应的日期（YYYY-MM-DD）
        # 日志等级校验，非法值默认INFO
        self._log_level = log_level.upper() if log_level.upper() in ["DEBUG", "INFO", "ERROR"] else "INFO"
        # 初始化当前日志文件和日期
        if log_file:
            self._update_log_file()  # 根据当前日期更新日志文件路径
